...
def test_replay_clip_can_attach_to_case(tmp_path, monkeypatch):
    ...
    case_service = type("CaseSvc", (), {"add_evidence": lambda self, case_id, payload, actor='system': attached.append((case_id, payload, actor))})()

    monkeypatch.setattr("app.services.replay_clip_service.get_stream_session_manager", lambda: session_manager)
    monkeypatch.setattr("app.services.replay_clip_service.get_case_service", lambda: case_service)
    ...
    assert response.attached_case_id == "case_1"
    assert attached and attached[0][0] == "case_1"
